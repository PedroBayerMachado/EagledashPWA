...
export default function Notes({ state, onUpdateFolders }: NotesProps) {
  ...
  return (
    <div className="h-[calc(100vh-12rem)] lg:h-[calc(100vh-16rem)] flex flex-col lg:flex-row gap-6 relative overflow-hidden">
      {/* Sidebar - Folders & Notes */}
      <div className={`w-full lg:w-80 flex flex-col gap-4 transition-all duration-300 ${selectedBlockId && 'hidden lg:flex'}`}>
      ...
      {/* Editor Area */}
      <div className={`flex-1 card bg-bg-light dark:bg-black-soft flex flex-col p-0 overflow-hidden transition-all duration-300 ${!selectedBlockId && 'hidden lg:flex'}`}>
        {selectedBlock ? (
          <>
            {/* Toolbar */}
            <div className="p-3 border-b border-gray-light dark:border-gray-mid flex items-center justify-between bg-bg-ice dark:bg-gray-charcoal">
              <div className="flex items-center gap-2">
                <button 
                  onClick={() => setSelectedBlockId(null)}
                  className="lg:hidden p-2 hover:bg-gray-light dark:hover:bg-gray-mid rounded transition-colors mr-2"
                >
                  <ChevronRight size={20} className="rotate-180" />
                </button>
                <div className="flex items-center gap-1 mr-2 sm:mr-4">
                  <Type size={16} className="opacity-40 hidden sm:block" />
                  <select 
                ...
                <button className="p-2 hover:bg-gray-light dark:hover:bg-gray-mid rounded transition-colors hidden sm:block">
                  <Bold size={18} />
                </button>
                <button className="p-2 hover:bg-gray-light dark:hover:bg-gray-mid rounded transition-colors hidden sm:block">
                  <Italic size={18} />
              ...
              <div className="flex items-center gap-2">
                <span className="text-[10px] opacity-40 uppercase font-bold mr-2 hidden sm:block">
            ...
            <div className="flex-1 p-4 sm:p-8 overflow-y-auto">
              <input 
                ...
                className="w-full text-2xl sm:text-3xl font-bold bg-transparent outline-none mb-4 sm:mb-6"
              ...
              <textarea 
                ...
                className="w-full h-full bg-transparent outline-none resize-none leading-relaxed min-h-[50vh]"
